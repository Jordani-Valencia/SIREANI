...
class CActualizaciones6 extends CI_Controller {

  function __construct(){
		parent::__construct();
		$this->load->model('MAct6');
		$this->load->library('session');
	}


	public function proyVida($alumno){
    ...
    $arr_proy=array(
      ...
      $this->MAct6->delProyecto($alumno);

    $id_proy=$this->MAct6->insGetProyecto($arr_proy);
    $id=$id_proy->id_proy_vida;

    $this->MAct6->delOpcionProy($id);
    if (isset($_POST['instalacion'])) {
      $arr_OPV= array(
        "id_opcion_pv"=>1,
        "id_proy_pv"=>$id);
      $this->MAct6->insProyOpcion($arr_OPV);
    }
    ...
    if ($_POST['especificar14']!="") {
      ...
      $nuevo=$this->MAct6->nuevaOpcion($arr_OPV);
      $arr_OPV= array(
    ...
    $arr_perfil=array(
      ...
      $this->MAct6->delPerifl($alumno);
    $this->MAct6->inserPerifl($arr_perfil);
